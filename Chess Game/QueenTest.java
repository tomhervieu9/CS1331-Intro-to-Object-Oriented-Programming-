...
public class QueenTest extends TestCase {
    ...
    public void testMoves() {
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                Set<Move> moves = queen.generateMoves(new Position(i, j));
                assertNotNull(moves);
                validateMoves(moves);
            }
        }
    ...
    public void randomCentralPositionMovesTest() {
        ...
        Set<Move> generated = queen.generateMoves(startPos);
        assertFalse("Generated Queen moves are fewer than actual",
            generated.size() < validMoves.size());
        assertFalse("Generated Queen moves are greater than actual",
            generated.size() > validMoves.size());
